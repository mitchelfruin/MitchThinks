...
For some values of $a$ SAY WHICH VALUES the system described by the logistic map converges on a stable equilibrium. 

To see what this process looks like over time we can simulate populations in which $a = 2$ and see what happens.

We'll simulate 100 generations for 3 different populations each with a random starting population proportion.

```{r}
stable_sim <- multiple_map(x_opts = round(runif(3), 3),
                           a_opts = 2,
                           n_opts = 100)
...
We see that regardless of the starting point, if $a=2$, then the population converges on a stable equilibrium proportion of $0.5$. 
...
log_map_df %>%
  ...
  geom_line() +
  scale_x_continuous("Population proportion at t") +
  scale_y_continuous("Population\nproportion\nat t+1",
                     breaks = c(seq(0, 1.25, 0.25))) +
...
The graph neatly illustrates the idea of density dependence that I described verbally before.

So now imagine starting anywhere on the x-axis, you'll always end up at 0.5. 

Graph with arrows?

## Period doubling

For other values of $a$, the logistic map exhibits cycles. 
...
```{r}
cycle_sim <- multiple_map(x_opts = round(runif(3), 3), a_opts = 3, n_opts = 100)
...
Now for the really interesting part. For some values of $a$, the logistic map is **chaotic**. This means it shows no pattern at all. NEED PROPER DEFINITION HERE. 

Chaos simulation, a = 3.65

```{r}
chaos_sim <- multiple_map(x_opts = round(runif(3), 3),
                          a_opts = 3.65,
                          n_opts = 100)
...
May summarised all of the behaviour we've seen so far into a single graph called a bifurcation diagram. 
...
Fortunately, there's been more than 40 years' worth of technological advancement since May's paper. This means that even a no-name chump can build an interactive tool that hopefully makes May's point a little easier to grasp.
