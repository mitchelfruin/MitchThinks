...
When Arthur was working at the Santa Fe Institute in 1993, there was a bar called El Farol with Irish music on thursday nights. He noted that people would go if they expected few people to be there, but avoided it if they expected it to be too busy.
...
We've seen that the mean attendance converges on the threshold value. It's not immediately obvious why this occurs. It turns out to be because the hypotheses (with the selection pressure of agents choosing their most accurate hypothesis) coevolve into an ecology in which $(100-X)\%$ predict values above the threshold and $X\%$ predict values below the threshold. To illustrate this consider 70:30 example with default threshold of 60.

One interesting result which Arthur highlights is that whilst the population of active hypotheses (the ones that are most accurate and thus used in a given round) splits into this $(100-X):X$ ratio, it keeps changing its membership continually. He liken this to a forest whose "contours do not change, but whose individual trees do". To illustrate this point I chose 5 individuals randomly from the original default simulation and plotted their individual predictions over the 100 weeks.
...
It's different people below the threshold each time.
